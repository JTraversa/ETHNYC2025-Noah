...
function ManageTab() {
  ...
  // Update beneficiary contract write
  const { data: beneficiaryHash, writeContract: writeBeneficiary, isPending: isUpdatingBeneficiary } = useWriteContract();
  const { isLoading: isBeneficiaryConfirming, isSuccess: isBeneficiarySuccess } = useWaitForTransactionReceipt({
  ...
  // Refetch ark data after successful ping, update, beneficiary change, or destroy
  useEffect(() => {
    if (isPingSuccess || isUpdateSuccess || isBeneficiarySuccess || isDestroySuccess) {
      refetch();
      refreshActivity(address, chainId);
      if (isUpdateSuccess) {
  ...
  }, [isPingSuccess, isUpdateSuccess, isBeneficiarySuccess, isDestroySuccess, refetch, address, chainId]);
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (!response.ok) {
          // 404 or other error means no arks found - destruction is complete
        ...
        <div className="bg-gradient-to-r from-slate-50 to-slate-100/50 rounded-xl p-4 border border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs text-slate-500">Beneficiary</div>
              </div>
            </div>
            <button
              onClick={() => setIsEditingBeneficiary(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-emerald-500 hover:text-white bg-emerald-50 hover:bg-gradient-to-r hover:from-emerald-500 hover:to-teal-500 border border-emerald-200 hover:border-transparent rounded-lg transition-all transform hover:scale-105 active:scale-100 hover:shadow-md hover:shadow-emerald-500/20"
            >
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-slate-800">Edit Beneficiary</h3>
                  <p className="text-xs text-slate-400">Set a new beneficiary address</p>
                </div>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-4 bg-gradient-to-r from-slate-50 to-slate-100/50 rounded-xl border border-slate-100 mb-4">
              <div className="text-xs text-slate-500 mb-1">Current Beneficiary</div>
            ...
            <div className="mb-4">
              <label className="block text-xs font-medium text-slate-600 mb-2">New Beneficiary Address</label>
              <input
                type="text"
                value={newBeneficiary}
                onChange={(e) => setNewBeneficiary(e.target.value)}
                placeholder="0x..."
                className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 focus:bg-white outline-none transition-all text-sm font-mono"
              />
              {newBeneficiary && !isValidAddress(newBeneficiary) && (
                <p className="text-xs text-red-500 mt-2">Invalid address format</p>
              )}
            </div>

            <button
              onClick={handleUpdateBeneficiary}
              disabled={!isValidAddress(newBeneficiary) || isUpdatingBeneficiary || isBeneficiaryConfirming}
              className="w-full flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold shadow-lg shadow-emerald-500/30 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none transition-all transform hover:scale-[1.02] active:scale-[0.98] disabled:transform-none"
            >
              {isUpdatingBeneficiary || isBeneficiaryConfirming ? (
                <>
                  <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                  {isUpdatingBeneficiary ? 'Confirm in Wallet...' : 'Updating...'}
                </>
              ) : (
                <>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
